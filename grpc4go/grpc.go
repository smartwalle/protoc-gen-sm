...
const (
	...
	poolPkgPath    = "github.com/smartwalle/grpc4go"
	suffix         = "4Go"
)
...
func (g *grpc) Generate(file *generator.FileDescriptor) {
	...
	poolPkg = string(g.gen.AddImport(poolPkgPath))

	g.P("var _ ", poolPkg, ".ClientConn")
	g.P()
...
func (g *grpc) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
	...
	// Client structure.
	g.P("type ", unexport(servName), "Client"+suffix+" struct {")
	g.P("cc *", poolPkg, ".ClientConn")
	g.P("}")
	g.P()
	...
	g.P("func New", servName, "Client", suffix, " (cc *", poolPkg, ".ClientConn) ", generator.CamelCase(service.GetName()), "Client {")
	g.P("return &", unexport(servName), "Client", suffix, "{cc}")
	g.P("}")
	g.P()
...
func (g *grpc) generateClientMethod(servName, fullServName, serviceDescVar string, method *pb.MethodDescriptorProto, descExpr string) {
	...
	g.P("func (c *", unexport(servName), "Client", suffix, ") ", g.generateClientSignature(servName, method), "{")
	if !method.GetServerStreaming() && !method.GetClientStreaming() {
		g.P("out := new(", outType, ")")
